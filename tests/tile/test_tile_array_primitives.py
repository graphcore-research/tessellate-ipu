...
def test__tile_put_sharded_prim__invalid_number_tiles():
    input = np.asarray([1, 2, 3], np.float32)
    tiles = (3, 4, 5, 6)
    with pytest.raises(AssertionError):
        tile_put_sharded_prim(input, tiles)


def test__tile_put_sharded_prim__no_jitting():
    input = np.asarray([1, 2, 3], np.float32)
    tiles = (3, 4, 5)
...
def test__tile_put_sharded_prim__device_jitting(backend):
    input = np.asarray([1, 2, 3], np.float32)
    tiles = (3, 4, 5)
