...
import jax
import numpy as np
from jax_ipu_research.tile import tile_put_replicated, tile_put_sharded, tile_map_primitive, tile_data_barrier
...
def compute_unsync_fn(data):
    ...
    for idx in range(sqrt_size):
        ...
        for _ in range(sqrt_size):
            ...
            gp1_data = tile_map_primitive(jax.lax.add_p, gp1_data, gp1_data)
            gp1_data = tile_map_primitive(jax.lax.mul_p, gp1_data, gp1_data)

            # Gp0 compute + comms.
            gp0_data = tile_map_primitive(jax.lax.add_p, gp0_data, gp0_data)
    ...
    gp0_data = tile_map_primitive(jax.lax.add_p, gp0_data, gp1_data)
