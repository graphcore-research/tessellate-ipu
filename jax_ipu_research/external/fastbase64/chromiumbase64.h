 ...
 */
inline std::string chromium_base64_encode(const std::string& s)
{
    std::string x(chromium_base64_encode_len(s.size()), '\0');
    size_t d = chromium_base64_encode(const_cast<char*>(x.data()), s.data(), s.size());
    if (d == MODP_B64_ERROR) {
        x.clear();
    } else {
...
inline std::string chromium_base64_decode(const std::string& s)
{
    std::string x(chromium_base64_decode_len(s.size()), '\0');
    size_t d = chromium_base64_decode(x.data(), s.data(), s.size());
    if (d == MODP_B64_ERROR) {
        x.clear();
    } else {
        x.erase(d, std::string::npos);
